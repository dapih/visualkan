...
`npm pack --dry-run` must list only `visualkan.mjs`, `scripts/`, `references/`, `skill/`, `README.md`, `CHANGELOG.md`, `LICENSE`, and `package.json`. Anything else means the `files` field in `package.json` is broken. A missing `scripts/` means every install ships without its Runtime.
...
Add an entry to the Version History table in [CHANGELOG.md](CHANGELOG.md), and a section describing what changed.
...
Also confirm that the install wrote a working Runtime path. This costs nothing:

```bash
visualkan status                    # no STALE and no no-runtime rows
```

Then run the exact command the skill body now carries, from an unrelated directory, and confirm it prints the catalog.
